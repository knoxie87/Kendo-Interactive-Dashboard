    ...
    const [displayNav,setNavbar] = useState(false)
     {
        return (
            ...
            <div className="bg-blue-800 text-blue-100 w-64 space-y-6 py-7">
              ...
              {displayGameWidgets ? 
                <GameWidgets addWidget = {props.addWidget}/>
              : null}

              <div className="">
              <a href="#" className="block w-full flex items-center py-2.5 px-4 rounded transition duration-200 hover:bg-blue-700 hover:text-white " onClick={() => setWeatherWidgets(s =>!s)}>Job Widgets 
              <svg  xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 ml-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
              </a>
              </div>
              {displayWeatherWidgets ? 
                <JobWidget addWidget = {props.addWidget}/>
              : null}
              </nav>
